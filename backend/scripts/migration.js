...
function getAppliedMigrations() {
  const res = runWranglerQuery(
    'SELECT name as "migration_name" FROM prisma_migrations ORDER BY registeredAt;'
  );
  if (!Array.isArray(res) || !res[0] || !Array.isArray(res[0].results))
    return [];
...
function runMigrations() {
  ...
  console.log(
    `[migration] Applying ${pending.length} migration(s): ${pending.join(', ')}`
  );
  ...
  for (const migration of pending) {
    ...
    if (!fs.existsSync(sqlFile)) {
      console.warn(
        `[migration] Skipping ${migration}: migration.sql not found`
      );
      continue;
    }
